...
Root.prototype.load = function load(filename, options, callback) {
    ...
    function process(filename, source) {
        try {
            if (util.isString(source) && source.charAt(0) === "{") {
                console.log("in the is Strinng")
                source = JSON.parse(source);
            }
            if (!util.isString(source)) {
                console.log("in the !is Strinng if")
                self.setOptions(source.options).addJSON(source.nested);
            } else {
                console.log("in the !is Strinng else")
                ...
                if (parsed.weakImports) {
                    console.log("parsed.weakImports")
            ...
            console.log("yes i am in the catch")
            finish(err);
        }
        if (sync && !queued) {
    ...
    function fetch(filename, weak) {
        ...
        if (self.files.indexOf(filename) > -1)
            return;
        console.log("self.files.push")
        self.files.push(filename);

        // Shortcut bundled definitions
        if (filename in common) {
            if (sync) {
                console.log("filename in common if")
                process(filename, common[filename]);
            }
            else {
                console.log("filename in common else")
                ++queued;
                setTimeout(function () {
                    --queued;
                    process(filename, common[filename]);
        ...
        if (sync) {
            ...
            // filename = '/storage/emulated/0/Download/Babybath.proto'
            // fs.readFile(filename).then((string) => {
            //     console.log(string);
